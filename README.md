...
For concurrent processing, you can specify any of the three workers—`:ractor`, `:process`, or `:thread`—using the `concurrency_method` option. Alternatively, choose `:none` for serial execution.
...
```ruby
Pbt.assert(concurrency_method: :ractor) do
...
```ruby
Pbt.assert(concurrency_method: :process) do
  Pbt.property(Pbt.integer) do |n|
    # ...
  end
end
...
```ruby
Pbt.assert(concurrency_method: :thread) do
  Pbt.property(Pbt.integer) do |n|
    # ...
  end
end
...
```ruby
Pbt.assert(concurrency_method: :none) do
  Pbt.property(Pbt.integer) do |n|
    # ...
  end
end
